...
however we do not have a (polymorphic or monomorphic) builtin constructtor to create a `BuiltinPair`
...
Possible solutions: Some more aggressive PIR inlining, or rewriting the PlutusTx TH code may help.
    ...
    (A (I (I (B builtin) tyFst) tySnd)
     (A (B UnConstrData) (A (A (B ConstrData) arg1) arg2))) ->
        case builtin of
            ...
            SndPair -> (tyFst,arg1) `seq_` pure arg2

            -- fstPair({t1}, {t2}, unConstr(constrData(i, data))) = let !gen = i in data `seq` gen
            FstPair ->  do
                  (genVar, genLetIn) <- strictLet tyFst arg1
                  genLetIn <$>
                      ((tySnd, arg2) `seq_` pure genVar)
            _ -> pure t
    _ -> pure t

  where
      infixr 5 `seq_` -- so it has more precedence than <$>
      seq_ = seqA binfo vinfo
